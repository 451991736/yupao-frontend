<template>
    <div>
        <van-card id="teamCardList" v-for="team in props.teamList" :thumb="mouse" :desc="team.description"
            :title="`${team.name}`">
            <template #tags>
            ...
            <template #bottom>
                ...
                <div v-if="team.expireTime">
                    {{ '过期时间: ' + team.expireTime.substring(0, 16) }}
                </div>
                <div>
                    {{ '创建时间: ' + team.createTime.substring(0, 16) }}
                </div>
            </template>
            <template #footer>
...
import mouse from '../assets/ikun.png'
